(ns aoc.core
  (:require
    ...
    [clojure.core.async :as a :refer [>! <! <!! >!! put! poll!]]
    [clojure.edn :as edn]
    [aoc.util :as util]
...
;; 9-1
(defn get-mem [m ptr]
  (let [{:keys [program mem]} m]
    (if (>= ptr (count program))
      (get mem ptr 0)
      (get program ptr))))

(defn set-mem [m ptr value]
  (let [{:keys [program ]} m]
    (if (>= ptr (count program))
      (assoc-in m [:mem ptr] value)
      (assoc-in m [:program ptr] value))))

(defn get-val [m arg-pos]
  (let [{:keys [args modes rbase]} m
        arg  (get args arg-pos)
        mode (get modes arg-pos)]
    (condp = mode
      0 (get-mem m arg)
      1 arg
      2 (get-mem m (+ arg rbase)))))

(defn get-ptr [m arg-pos]
  (let [{:keys [args modes rbase]} m
        arg  (get args arg-pos)
        mode (get modes arg-pos)]
    (condp = mode
...
(defn op-1 [m]
  (let [arg1 (get-val m 0)
        arg2 (get-val m 1)
        arg3 (get-ptr m 2)]
    (-> m
        (set-mem arg3 (+ arg1 arg2))
        (update :instr-ptr + 4))))
...
(defn op-3 [m]
  (let [arg (get-ptr m 0)
        _ (println "please input value? ")
        value (edn/read-string (read-line))]
    (-> m
        (set-mem arg value)
        (update :instr-ptr + 2))))

(defn op-4 [m]
  (let [value (get-val m 0)]
...
(defn execute-opcode [instructions state]
  ;(a/go-loop [m state]
  (loop [m state i 0]
    (let [{:keys [program instr-ptr]} m
          instr   (get program instr-ptr)
          ...
          rtn     (condp = opcode
                    ...
                    3 (op-3 info)
                    4 (op-4 info))]
                    ;3 (let [arg (get-val info 0)]
                    ;    (-> m
                    ;        (set-mem arg (<! (:in-ch info)))
                    ;        (update :instr-ptr + 2)))
                    ;4 (let [value (get-val info 0)]
                    ;    (>! (:out-ch info) value)
                    ;    (-> (update m :instr-ptr + 2))))]
      (if-not (or (:halt rtn) (< instructions (dec i)))
        (recur rtn (inc i))
        (println rtn)))))


#_(execute-opcode (init-program [109 10 203 10 204 10 99]))
#_(execute-opcode (init-program [109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99]))
#_(execute-opcode (init-program [1102 34463338 34463338 63 99]))
#_(execute-opcode (init-program [104,1125899906842624,99]))
#_(execute-opcode 14 (init-program (into [] (edn/read-string (slurp "resources/9-1.edn")))))

#_ (take 50 (edn/read-string (slurp "resources/9-1.edn")))

#_ (-> (edn/read-string (slurp "resources/9-1.edn"))
       (->> (take 4))
       (vec)
       (into [99 21101 0 27 1 21101 0 913 0 1106 0 920 21201 1 53612 1 204 1 99 109 3 1207 -2 3 63 1005 63 962 21201 -2 -1 1 21102 940 1 0 1106 0 920 21202 1 1 -1 21201 -2 -3 1 21101 955 0 0 1106 0 920 22201 1 -1 -2 1105 1 966 21201 -2 0 -2 109 -3 2106 0 0])
       (init-program)
       (execute-opcode))
